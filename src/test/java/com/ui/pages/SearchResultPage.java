...
public class SearchResultPage extends BrowserUtility{
	...
	public boolean isSearchTermProductList(String searchTerm) {
		List <String> keywords = Arrays.asList(searchTerm.toLowerCase().split(" "));
		List<String> productNameLists = getAllVisibleText(ALL_PRODUCT_LISTS_NAME_LOCATOR);
		boolean result = productNameLists.stream().anyMatch(name -> (keywords.stream().anyMatch(name.toLowerCase()::contains)));
		return result;
	}
	
	public ProductDetailsPage clickOnTheProductIndex(int index) {
		scrollDownScreen();
